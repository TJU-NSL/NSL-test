...
kv_cache = None # add for kv cache
...
def gelu(x):
    ...
    res = 0.5 * x * (1 + torch.tanh(math.sqrt(2 / torch.pi) * (x + 0.044715 * torch.pow(x, 3))))
    return res
...
def ffn(x, mlp):  # [n_seq, n_embd] -> [n_seq, n_embd]
    ...
    a = gelu(linear(x, w_b1))  # [n_seq, n_embd] -> [n_seq, 4*n_embd]
    x = linear(a, w_b2)  # [n_seq, 4*n_embd] -> [n_seq, n_embd]
    return x
...
def attention(q, k, v, mask):  # [n_q, d_k], [n_k, d_k], [n_k, d_v], [n_q, n_k] -> [n_q, d_v]
    ...
    global kv_cache # add for kv cache
    if kv_cache.full(): # add for kv cache
        k_cache, v_cache = kv_cache.get() # add for kv cache
        k = torch.cat([k_cache, k], dim=0) # add for kv cache
        v = torch.cat([v_cache, v], dim=0) # add for kv cache
    kv_cache.put((k, v)) # add for kv cache
    
    scaled_dot_product = torch.matmul(q, k.transpose(-2, -1)) / math.sqrt(q.shape[-1])
    if mask is not None: # add for kv cache
        scaled_dot_product += mask # modify for kv cache
...
def mha(x, attn, n_head):  # [n_seq, n_embd] -> [n_seq, n_embd]
    ...
    if not kv_cache.full(): # add for kv cache
        causal_mask = (torch.ones(x.size(0), x.size(0)) - torch.tril(torch.ones(x.size(0), x.size(0)))) * -1e10 # modify for kv cache
    else:   # add for kv cache
        causal_mask = None # add for kv cache
...
def generate(inputs, params, n_head, n_tokens_to_generate):
    ...
    global kv_cache # add for kv cache
    kv_cache = queue.Queue(n_head * len(params['blocks'])) # add for kv cache
